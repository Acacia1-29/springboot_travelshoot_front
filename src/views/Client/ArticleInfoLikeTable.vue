  ...
  <template>
    <div class="app-container">
        ...
        <el-dialog title="资讯点赞编辑对话框" :visible.sync="editorShow" width="80%" :lock-scroll="true" height="800px">
        ...
        <data-table ref="tableId" url="/ArticleInfoLike/List" :column="dataColum" :where="{ LikeUserId: userId }">
            <template v-slot:header>
            </template>
            <template v-slot:operate="scope">
                <el-button type="text" @click="Look(scope.row.ArticleInfoId)">浏 览</el-button>
                <el-button type="text" @click="ShowDeleteModal(scope.row.Id)">取 消 点 赞</el-button>
            </template>
        </data-table>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async ShowEditModal(Id) {
            ...
            let { Data } = await this.$Post(`/ArticleInfoLike/Get`, { Id: Id });
            this.formData = Data;
        ...
        async CreateOrEditForm() {
            ...
            this.$refs.editModalForm.validate(async valid => {
                if (valid) {
                    this.lock = 1;
                    var { Success } = await this.$PostLoading(`/ArticleInfoLike/CreateOrEdit`, this.formData);
                    this.lock = 0;
                    if (Success) {
        ...
        async ShowDeleteModal(Id) {
            await this.$PostDelete(`/ArticleInfoLike/Delete`, { Id: Id });
            this.$refs.tableId.reload();
        },
        /**
         * 批量删除的时候会触发
        ...
        async ShowBatchDeleteModal() {
            ...
            await this.$PostDelete(`/ArticleInfoLike/BatchDelete`, { Ids: ids });
            this.$refs.tableId.reload();
        },
        /**
         * 点击表格搜索按钮会触发
